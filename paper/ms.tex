...
\textsc{treecorr} (Figure~\ref{fig:rho}). The picture is mixed and
instructive. On the size-leakage terms $\rho_3$ and $\rho_4$ our amortized model
is a factor of $2$--$3$ \emph{below} both per-exposure baselines, consistent
with its smaller per-star size scatter. On the shape terms $\rho_1$ and
especially $\rho_2=\langle e\,\delta e\rangle$, however, it is larger---a roughly
flat ${\sim}5\times10^{-4}$ plateau that persists across the CCD, whereas
\textsc{PIFF} and \textsc{PSFEx} decorrelate toward ${\lesssim}10^{-5}$ at
arcminute scales. This is not a global ellipticity offset (our mean residual
$\delta e$ is in fact smaller than the baselines') but a signature of mild
\emph{ellipticity shrinkage}: the learned model regresses predicted shape toward
the population mean, so $\delta e$ correlates with the true PSF shape exactly
where the PSF is most elliptical. Per-exposure fits, which solve each field
independently, avoid this by construction. The amortized model thus trades lower
per-star scatter and size leakage for a coherent shape-leakage term that a
weak-lensing pipeline would need to monitor; reducing it (e.g.\ via the
point-source-context conditioning of \S\ref{sec:sim}, or an explicit
shape-debiasing calibration) is a clear target for future work.
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{figures/fig_rho_stats.pdf}
\caption{PSF residual $\rho$-statistics on reserved stars (all bands), per
method. Our model (red) is below the baselines on the size-leakage terms
$\rho_3,\rho_4$ but above them on the shape terms $\rho_1,\rho_2$, reflecting a
coherent ellipticity-shrinkage residual (\S\ref{sec:rho}).\label{fig:rho}}
\end{figure*}
