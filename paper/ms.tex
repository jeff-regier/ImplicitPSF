...
CI per arm. Left: recovered ellipticity is unbiased ($|\Delta e_1|\lesssim0.004$) for all
three methods. Right: the methods separate on recovered \emph{size}---our model runs
${\sim}8\%$ small (the PSF-core under-concentration), while \textsc{PIFF} and the truth arm
are near zero (\S\ref{sec:galrec}).\label{fig:galrec}}
\end{figure*}
...
\colhead{$\Delta e_2$}}
\startdata
Truth (validation) & $-1.2\%$ & $-0.000$ & $-0.011$ \\
\textbf{This work}  & $-7.6\%$ & $-0.001$ & $-0.008$ \\
