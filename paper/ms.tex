...
data, our PSF recovers galaxy \emph{ellipticity} without coherent bias
($|\Delta e_1|\lesssim0.01$, on par with \textsc{PIFF})---the weak-lensing-critical
quantity---while recovered galaxy \emph{sizes} run ${\sim}8\%$ small. We trace this to the
coordinate decoder under-resolving the sharp PSF core and reduce it with a sharper
frequency encoding. Color
...
arbitrary position, color, flux, and resolution. Amortizing a learned prior in place of
per-exposure curve fitting is the central idea: it lets the model pool information across
...
(the anchor's own image) recovers near-zero ellipticity bias, validating the protocol.
On the $r$-band test exposures (Table~\ref{tab:galrec}), all three methods recover galaxy
...
separate instead on recovered \emph{size}: \textsc{PIFF} is slightly large ($+3.4\%$),
...
PSF core; \S\ref{sec:discussion} diagnoses this and shows it shrinks with a sharper
coordinate encoding. The size deficit is the main residual systematic of our model;
recovered ellipticity, the consequential quantity for shear, is unbiased.
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/fig_galrec.pdf}
\caption{Galaxy ellipticity recovery on real data (star-anchored injection). All methods
recover galaxy $e_1$ with low bias ($|\Delta e_1|\lesssim0.01$); they differ in recovered
\emph{size}, not ellipticity (\S\ref{sec:galrec}).\label{fig:galrec}}
\end{figure}



\begin{deluxetable}{lccc}
\tablecaption{Star-anchored galaxy injection--recovery ($r$-band test exposures). The
truth arm validates the protocol. All methods recover ellipticity with low bias; they
separate on recovered size.\label{tab:galrec}}
\tablehead{\colhead{Arm} & \colhead{Size bias} & \colhead{$\Delta e_1$} &
\colhead{$\Delta e_2$}}
\startdata
Truth (validation) & $-1.2\%$ & $+0.008$ & $-0.004$ \\
\textbf{This work}  & $-7.8\%$ & $-0.005$ & $+0.002$ \\
\textsc{PIFF}       & $+3.4\%$ & $+0.004$ & $+0.004$ \\
\enddata
\end{deluxetable}
...
biases recovered galaxy sizes ${\sim}8\%$ low (\S\ref{sec:galrec}). We attribute this to
...
% TODO: DES data-use acknowledgment (the standard DES public-data statement) -- pending text.
