...
\shorttitle{An Implicit Effective-PSF Field for DES}
...
\title{Learning the Effective PSF: An Amortized, Implicit PSF Field for
...
The network targets the \emph{pixel-convolved} (effective) PSF---the expected pixel
values a point source would produce---rather than a sub-pixel surface-brightness profile,
making the directly observable, identifiable quantity the regression target and the
exact kernel needed to forward-model galaxies.
