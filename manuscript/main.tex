...
flux-dependent effect---and a Monte Carlo EM that infers and subtracts each star's
contaminant posterior supplies the principled per-star correction and a calibrated
contamination model.
...
falling to $+0.0\pm0.4\%$ at $10^6$ ($n=260$ reserved stars over
$50$ held-out exposures)---and the recovered galaxy-size bias collapses correspondingly
(de-confounded $-6.2\%\to-1.3\%$). The clean window is broad: the size residual is flat
...
faint-neighbour contamination of the training stars is the cause; the Monte Carlo EM of
\S\ref{sec:mcem} removes that contamination explicitly and per-star, supplying a calibrated
contamination model and cleaning the small residual. The size bias is thus largely a
query-flux effect rather than an irreducible limitation; recovered ellipticity, the
consequential quantity for shear, is unbiased throughout.
\begin{figure*}
\centering
...
$\delta\mathrm{EE}(r{\le}2\,{\rm px})=-0.0078$ relative to truth at the median query flux,
and injected galaxies are recovered $-2.4\%$ small (the simulation analogue of the real
deficit, after differencing against recovery through the known clean PSF to cancel the
protocol floor). Because the simulation has no brighter-fatter, the model's only
flux-dependent effect is the contamination it learned, and querying its PSF at the clean
high-flux limit---with no cleaning at all---already drives $\delta\mathrm{EE}$ from
$-0.0078$ to $-0.0014$, i.e.\ essentially onto truth; this is the controlled,
truth-anchored counterpart of the real-data flux query in \S\ref{sec:galrec}. The Monte
Carlo EM of \S\ref{sec:mcem} removes the same contamination explicitly and per-star. Cleaning each training star against its inferred
