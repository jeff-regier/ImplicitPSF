...
level is thus learned from the many-star aggregate rather than assumed, and \S\ref{sec:sim} shows
the inferred $(\lambda,\alpha)$ recover the generator's together with the PSF and galaxy sizes. We place \emph{no} prior on the network weights $\theta$ --- we
...
(ii) \emph{M-step} --- increase $\hat Q$ by stochastic gradient descent, a generalized EM step
\citep{dempster1977em} since the network is not maximized exactly; operationally each minibatch
draws one of the $K$ imputations per star and subtracts it, training against the multiply
imputed cleaned stars so the contaminant posterior \emph{uncertainty} propagates into the fit
rather than a point estimate. (iii) Iterate until the PSF and the recovered galaxy sizes
