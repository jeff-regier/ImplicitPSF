...
with \textsc{PIFF}, while recovered \emph{sizes} run a few percent small. Higher-capacity
coordinate encodings reduce this size deficit, but controlled simulations with known PSFs do
not reproduce it at any decoder capacity, so it is specific to real data and its origin remains
under investigation. Color conditioning further removes a chromatic PSF-size systematic
...
\emph{ellipticity} with low bias ($|\Delta e_1|\lesssim0.01$): the pixel-convolved
...
it is not a generic spectral-bias limitation of the decoder. Its full origin is not yet isolated:
contamination of nominally-clean training stars by faint unresolved neighbours reproduces the sign
and \textsc{PIFF}'s relative robustness but only a fraction of the magnitude, and unmodeled real
PSF morphology is the leading remaining candidate. The size deficit is the main residual systematic
...
CI per PSF. Left: recovered ellipticity is unbiased ($|\Delta e_1|\lesssim0.004$) for all
...
\colhead{$\Delta e_2$}}
\startdata
True PSF            & $-1.2\%$ & $-0.000$ & $-0.011$ \\
\textbf{Neural PSF (ours)}  & $-7.6\%$ & $-0.001$ & $-0.008$ \\
\textsc{PIFF}       & $+1.1\%$ & $-0.004$ & $-0.012$ \\
\enddata
\end{deluxetable}
...
our approach requires many exposures to learn the field (in simulation, ellipticity
...
multilayer-perceptron decoder. Its full origin is under investigation---faint-neighbour
contamination of the training stars reproduces the sign and \textsc{PIFF}'s robustness but, in
simulation, only a fraction of the magnitude and without a clear dose-response, so unmodeled real
PSF morphology is the leading remaining candidate. It is the main residual systematic to address before a shear analysis. A separate, milder effect appears only in the
