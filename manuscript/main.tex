...
$\alpha$---are inferred jointly rather than assumed, and are recovered with calibrated
uncertainty: simulation-based calibration gives nominal $90\%$ credible-interval coverage
($1.00$ and $0.94$ for $\lambda$ and $\alpha$) with well-mixed chains (Gelman--Rubin
$\hat R<1.05$). Two diagnostics were decisive and are worth recording. First, the cleaning
...
With both addressed, the correction recovers the truth on all three axes---PSF concentration,
galaxy size, and the contamination hyperparameters.
