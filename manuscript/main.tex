...
On 32 $r$-band test exposures (640 anchors; Table~\ref{tab:galrec}), all three methods recover galaxy
\emph{ellipticity} with low bias ($|\Delta e_1|\lesssim0.016$): the pixel-convolved
...
galaxy-size bias collapses correspondingly: on $32$ matched held-out exposures
($n=640$ anchors) the de-confounded bias (implicit minus the true-PSF recovery) falls from
$-4.6\%$ at the median flux to $-0.9\%$ at the clean flux, i.e.\ essentially to the protocol
floor. The recovery is robust across all production seeds (the predicted PSF size returns to
within $1\%$ of bright clean stars at the clean flux for every seed, though the median-flux
deficit magnitude is itself somewhat seed-dependent). The clean
...
CI per PSF. Left: recovered ellipticity is low-bias ($|\Delta e_1|\lesssim0.016$) for all
three methods. Right: recovered \emph{size}. Queried at the median flux our model runs small
(contamination-broadening of the predicted PSF); queried at the clean high-flux limit it
recovers to near the true-PSF floor (\S\ref{sec:galrec}). \textsc{PIFF} is shown for
reference.\label{fig:galrec}}
\end{figure*}



\begin{deluxetable}{lccc}
\tablecaption{Star-anchored galaxy injection--recovery (32 $r$-band test exposures, 640 anchors). The
true-PSF recovery sets the protocol floor. All methods recover ellipticity with low bias;
they separate on recovered size. Rendering our model's PSF at the median query flux leaves
a size deficit (contamination-broadening, \S\ref{sec:galrec}); at the clean high-flux limit
it is largely removed (Table~\ref{tab:fluxsize}).\label{tab:galrec}}
\tablehead{\colhead{PSF} & \colhead{Size bias} & \colhead{$\Delta e_1$} &
\colhead{$\Delta e_2$}}
\startdata
True PSF                          & $-1.3\%$ & $-0.009$ & $+0.004$ \\
Neural PSF (median flux)          & $-5.8\%$ & $-0.006$ & $+0.005$ \\
\textbf{Neural PSF (clean flux)}  & $-2.2\%$ & $-0.011$ & $+0.005$ \\
\textsc{PIFF}                     & $+1.3\%$ & $-0.016$ & $+0.003$ \\
\enddata
\end{deluxetable}
...
is the contamination the flux-conditioned model has implicitly learned to separate. This
survives a deliberately adversarial test of the real selection: when the detectability
threshold is made to rise with host brightness---so that bright ``clean'' survivors retain
\emph{more} masked contamination, as the real SEP filter allows---the flux query still
recovers truth ($\delta\mathrm{EE}=-0.0059\to-0.0005$ from median to clean flux). The query
relies only on bright stars being \emph{fractionally} cleaner, which holds even when their
retained neighbours are absolutely brighter, so the recovery is not an artifact of assuming
bright stars are perfectly clean.
