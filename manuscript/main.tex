...
with \textsc{PIFF}, while recovered \emph{sizes} run a few percent small. We trace a
contributing cause to faint unresolved neighbours contaminating nominally-clean training
stars, and correct it with a Monte Carlo EM that infers and subtracts each star's
contaminant posterior before refitting the field. In controlled simulations the correction
is decisive: it returns the PSF concentration and the recovered galaxy sizes to truth, and
the contamination rate is recovered with calibrated uncertainty.
\placeholder{real-data size-deficit reduction after correction.}
...
\subsection{Contamination correction by Monte Carlo EM} \label{sec:mcem}

Reserved ``clean'' stars are never truly clean: sub-threshold blends add faint light to
their cutouts and bias the learned PSF toward an under-concentrated profile. We remove this
bias with a latent-variable model fit by expectation maximization \citep{dempster1977em}.

\paragraph{Model} For clean star $i$, the cutout $\mathbf{y}_i$ is a bright central source ---
the star, whose surface brightness is the effective PSF $\boldsymbol\pi_\theta(\mathbf{x}_i)$
scaled by an unknown amplitude $a_i$ --- plus faint nuisance sources and noise with the DES
inverse-variance weights $\mathbf{w}_i$,
\begin{equation}
\mathbf{y}_i = a_i\,\boldsymbol\pi_\theta(\mathbf{x}_i)
  + \textstyle\sum_{j} f_{ij}\,\mathbf{g}(\boldsymbol\mu_{ij},\Sigma_{ij}) + \boldsymbol\varepsilon_i .
\end{equation}
The nuisance sources are too faint to classify, so we model each as a single bivariate
Gaussian $\mathbf{g}(\boldsymbol\mu,\Sigma)$ of flux $f$, sub-pixel centroid $\boldsymbol\mu$,
and covariance $\Sigma = \Sigma_\theta(\mathbf{x}_i) + LL^\top$, where $\Sigma_\theta$ is the
current PSF second-moment matrix and $LL^\top\succeq0$ (Cholesky factor $L$) is a non-negative
intrinsic broadening. A contaminant is thus at least as broad as a point source and may be
extended and elliptical, with no star/galaxy dichotomy.

\paragraph{Latents, prior, and parameters} The nuisance configuration
$\mathbf{z}_i=\{(b_{ic},f_{ic},\boldsymbol\mu_{ic},L_{ic})\}$ lives on a per-pixel detection
grid, at most one source per cell ($b_{ic}\in\{0,1\}$): a fixed-dimensional mean-field
representation that avoids transdimensional moves. It carries an informative, physically
motivated prior $p(\mathbf{z})$ --- per-cell Bernoulli rate, truncated power-law flux (slope
and bright cutoff set by DES source counts and the detection limit that defines
``sub-threshold''), and a weak prior on $L$. This prior alone resolves the degeneracy between
a broad PSF with no contamination and a sharp PSF with faint contamination, and so sets the
magnitude of the correction. Crucially, its hyperparameters --- the per-cell detection rate
$\lambda$ and the flux power-law slope $\alpha$ --- are unknown on real data, so we do not fix
them: they are \emph{global} random variables shared across stars, with weakly-informative
hyperpriors (a conjugate Gamma rate; a Gamma slope), and are inferred jointly. The contamination
level is thus learned from the many-star aggregate rather than assumed; \S\ref{sec:sim} tests on
simulations with a known PSF field whether the inferred $(\lambda,\alpha)$ recover the generator's
together with the PSF concentration and galaxy sizes. We place \emph{no} prior on the network weights $\theta$ --- we
seek the maximum marginal-likelihood PSF, not a Bayesian network --- and none on the central
amplitudes $a_i$. These are parameters: $a_i$ is profiled in closed form and $\theta$ is fit
in the M-step. Only the counts-endowed nuisances are marginalized,
\begin{equation}
\hat\theta=\arg\max_\theta\sum_i\log\!\int p(\mathbf{y}_i\mid\theta,\mathbf{z}_i)\,
  p(\mathbf{z}_i)\,d\mathbf{z}_i ,
\end{equation}
with $a_i$ profiled inside the integrand.

\paragraph{Monte Carlo EM} The integral is intractable, so we use EM with the E-step
expectation replaced by a Monte Carlo average over contaminant draws \citep{wei1990mcem}. At
iteration $t$ with current PSF $\theta^{(t)}$: (i) \emph{E-step} --- for each star draw $K$
samples $\mathbf{z}_i^{(t,k)}\sim p(\mathbf{z}_i\mid\mathbf{y}_i,\theta^{(t)})$ by collapsed
Gibbs, profiling $a_i$ each sweep (it has no prior), marginalizing each flux in closed form,
resampling detection, centroid, and covariance per cell, and --- globally across all stars ---
resampling $(\lambda,\alpha)$ from the pooled contaminant sufficient statistics, a hierarchical
Gibbs step; the Monte Carlo objective is
$\hat Q(\theta\mid\theta^{(t)})=K^{-1}\sum_{i,k}\log p(\mathbf{y}_i\mid\theta,\mathbf{z}_i^{(t,k)})$.
(ii) \emph{M-step} --- increase $\hat Q$ by stochastic gradient descent warm-started from
$\theta^{(t)}$, a generalized EM step \citep{dempster1977em} since the network is not maximized
exactly; operationally each minibatch draws one of the $K$ imputations per star and subtracts it,
training against the multiply imputed cleaned stars so the contaminant posterior \emph{uncertainty}
propagates into the fit rather than a point estimate. (iii) Iterate until the PSF and the recovered galaxy sizes
(\S\ref{sec:sim}) stop changing; later iterations' sharper central template removes residual
contamination that the contaminated initial template leaves. We increase $K$ across iterations
and monitor Monte Carlo error \citep{booth1999mcem,levine2001mcem}; a stochastic-approximation
variant \citep{delyon1999saem} amortizes the per-iteration resampling cost.

\paragraph{Sampler validation} Two checks gate the E-step. \emph{Calibration}: simulation-based
calibration --- draw $\mathbf{z}$ from the prior, synthesize a stamp, infer, and require nominal
credible-interval coverage of the total contaminant flux and count (this caught three sampler
bugs in development). \emph{Mixing}: per star we run dispersed-initialization chains and report
the Gelman--Rubin $\hat R$ and effective sample size of the flux and count summaries; the E-step
proceeds only where $\hat R\!\approx\!1$ with adequate effective sample size, so a poorly mixing
chain cannot silently corrupt the fit.
...
while our recovered sizes are ${\sim}8\%$ small. This is \emph{not} a second-moment size
error (rendered at a fiducial flux, the model's PSF size is accurate at both star and
star-free positions, within $1\%$), but a profile-shape effect: the model's PSF is
slightly \emph{under-concentrated}, carrying ${\sim}2\%$ less encircled energy in the
central few pixels than the empirical PSF, which biases recovered galaxy sizes low. The
magnitude is architecture-dependent (Table~\ref{tab:galrec} reports the baseline; higher-capacity
coordinate encodings---random Fourier features, an explicit analytic core---roughly halve it,
\S\ref{sec:discussion}), pointing to an under-resolution component. It is, however, specific to
real data: controlled simulations with known PSFs do not reproduce it at any decoder capacity, so
it is not a generic spectral-bias limitation of the decoder. We attribute it, at least in part, to
faint unresolved neighbours contaminating the nominally-clean training stars: adding such
contamination to a simulation reproduces both the PSF under-concentration and the recovered-size
deficit, and the Monte Carlo EM of \S\ref{sec:mcem} removes both there (\S\ref{sec:sim}). The
direct test on real data---applying the identical cleaner and sampler to the real training stars and
re-running this injection--recovery experiment---is reported here:
\placeholder{real-data recovered galaxy-size bias before vs.\ after the contamination correction.}
The size deficit is the main residual systematic
of our model; recovered ellipticity, the consequential quantity for shear, is unbiased.
\begin{figure*}
\centering
...
three methods. Right: the methods separate on recovered \emph{size}---our model runs
${\sim}8\%$ small (the PSF-core under-concentration), while \textsc{PIFF} and the
true PSF are near zero (\S\ref{sec:galrec}).\label{fig:galrec}}
\end{figure*}
...
they separate on recovered size.\label{tab:galrec}}
...
\paragraph{Contamination correction by Monte Carlo EM} Adding the faint-neighbour
contamination of \S\ref{sec:galrec} to the training stars reproduces the size deficit in
simulation, where the truth PSF is known exactly: the learned PSF is under-concentrated by
$\delta\mathrm{EE}(r{\le}2\,{\rm px})=-0.0072$ relative to truth, and injected galaxies are
recovered $-2.4\%$ small (the simulation analogue of the real deficit, after differencing
against recovery through the known clean PSF to cancel the protocol floor). The Monte Carlo
EM of \S\ref{sec:mcem} removes it. Cleaning each training star against its inferred
contaminant posterior and refitting the field drives the PSF concentration to truth
($\delta\mathrm{EE}\to0$ at the fixed point, converged over EM iterations and stable across
five seeds) and shrinks the recovered galaxy-size bias from $-2.4\%$ to $-0.8\%$. The global
contamination hyperparameters---the per-stamp rate $\lambda$ and the flux power-law slope
$\alpha$---are inferred jointly rather than assumed. The inference is calibrated:
simulation-based calibration gives nominal $90\%$ credible-interval coverage ($1.00$ and
$0.94$ for $\lambda$ and $\alpha$) with well-mixed chains (Gelman--Rubin $\hat R<1.05$).
Run on the simulation itself, it recovers the effective contamination rate---the quantity
that sets the deficit---to within its uncertainty ($\lambda_{\rm true}=0.56$ per stamp,
posterior median $0.59$, $90\%$ CI $[0.50,0.67]$); the flux slope is recovered flatter than
injected ($\alpha_{\rm true}=1.5$, posterior $\approx1.1$), as expected when the detectable
contaminants are biased bright relative to the source population by incomplete detection of
the faintest neighbours. The EM marginalizes the slope, so this bias does not propagate to
the PSF. Two diagnostics were decisive and are worth recording. First, the cleaning
template must be rendered at each star's \emph{sub-pixel} centre: an integer-grid template
is offset from the star and the sampler removes the resulting dipole residual as spurious
contamination, over-sharpening the PSF and---because the cleaned PSF re-enters the next
E-step---running the EM away from truth; comparing the sampler's bias against the \emph{known}
PSF as the template isolated this. Second, the contaminant prior must exclude sources fainter
than the per-pixel noise, which are unidentifiable from noise; including them produced
per-cell false detections that biased the inferred rate high, caught by the calibration test.
With both addressed, the correction recovers the truth on the two axes that matter for
weak lensing---PSF concentration and recovered galaxy size---driven by an inferred
contamination rate that itself matches the simulation.
...
biases recovered galaxy sizes a few percent low (\S\ref{sec:galrec}). Higher-capacity coordinate
encodings reduce it: widening the decoder's Fourier frequency basis cuts the encircled-energy
residual at $r{=}2$\,px (${\sim}2\%\to0.1\%$) and the galaxy-size bias by ${\sim}30\text{--}50\%$,
and an explicit analytic core does likewise, indicating an under-resolution component. It is,
however, \emph{specific to real data}: controlled simulations with known PSFs do not reproduce the
deficit at any decoder capacity, ruling out a generic spectral-bias limitation of the
multilayer-perceptron decoder. We identify faint-neighbour contamination of the
training stars as a---likely the dominant---cause: it reproduces the under-concentration in
simulation, and the Monte Carlo EM of \S\ref{sec:mcem}, which infers and subtracts each star's
contaminant posterior before refitting, removes it there, returning the PSF concentration and the
recovered galaxy sizes to truth (\S\ref{sec:sim}). The identical cleaner and sampler apply to the
real training stars; \placeholder{real-data outcome of the correction.} It is the main residual
systematic to address before a shear analysis. A separate, milder effect appears only in the
...
correcting a chromatic systematic and remaining robust in star-sparse exposures. Controlled simulations motivate
two training choices: a blend forward-model loss and point-source-only attention
context. The latter follows from diagnosing a spin-2 representation failure triggered
