...
their cutouts and bias the learned PSF toward an under-concentrated profile. The flux query
of \S\ref{sec:galrec} mitigates this implicitly---the flux-conditioned model has learned
that fainter stars are more contaminated, so its bright-limit PSF is nearly clean---but that
relies on extrapolating the model toward the bright end of its training range. Here we
instead remove the bias \emph{explicitly}, with a latent-variable model fit by expectation
maximization \citep{dempster1977em}, yielding a per-star correction and a calibrated
contamination model that does not depend on such extrapolation.
...
galaxy-size bias collapses correspondingly
...
faint-neighbour contamination of the nominally-clean training stars as the cause; the Monte
Carlo EM of \S\ref{sec:mcem} removes it explicitly and per-star, supplying a calibrated
contamination model and cleaning the residual. The practical prescription---render the
inferential PSF at the clean-flux limit---removes most of this, the main residual systematic
to control before a shear analysis.
...
with a Monte Carlo EM providing the principled per-star contamination correction. Controlled
simulations motivate two further training choices: a blend forward-model loss and
